...
public class Ball extends GraphicsItem {
    ...
    public void bounceFromPaddle(double hitPositionRatio) {
        hitPositionRatio = Math.max(-1, Math.min(1, hitPositionRatio));
        double angle = hitPositionRatio * Math.PI / 4; // Max zmiana kąta to 45 stopni (PI / 4)
        double currentAngle = Math.atan2(moveVector.getY(), moveVector.getX());
        double newAngle = Math.PI - currentAngle + 2 * angle;
        moveVector = new Point2D(Math.cos(newAngle), Math.sin(newAngle)).normalize();
    }
